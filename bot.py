import telebot  # Baş harp kiçeldildi
...
@app.route('/')
def home():
    return "Bot is running perfectly!"  # 4 sany probel bilen saga süýşürildi

@app.route('/adsgram_webhook', methods=['GET'])
def adsgram_webhook():
    user_id = request.args.get('user_id')  # 4 sany probel bilen saga süýşürildi
    status = request.args.get('status')    # 4 sany probel bilen saga süýşürildi
...
def run_flask():
    port = int(os.environ.get("PORT", 10000))  # Saga süýşürildi
    app.run(host='0.0.0.0', port=port)         # Saga süýşürildi

START_PHOTO = "https://images.unsplash.com/photo-1621416894569-0f39ed31d247?q=80&w=1000&auto=format&fit=crop"

def init_db():
    conn = sqlite3.connect("bot_users.db", check_same_thread=False)  # Saga süýşürildi
    cursor = conn.cursor()                                           # Saga süýşürildi
    cursor.execute("""
...
def handle_menu(message):
    ...
    if message.text == TEXTS[lang]['btn_earn']:  
        markup = types.InlineKeyboardMarkup()  
        adsgram_url = f"https://t.me/AdsgramRewardBot/start?startapp={ADSGRAM_BLOCK_ID}-{user_id}"
